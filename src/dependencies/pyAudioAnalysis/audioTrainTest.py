...
def featureAndTrain(listOfDirs, mtWin, mtStep, stWin, stStep, classifierType, modelName, computeBEAT=False, perTrain=0.90, confusionMatrixInPercent=True, verbosity=True):
    '''
    ...
    [features, classNames, _] = aF.dirsWavFeatureExtraction(listOfDirs, mtWin, mtStep, stWin, stStep, computeBEAT=computeBEAT, verbosity=verbosity)
    ...
    bestParam, bestAcc = evaluateClassifier(features, classNames, 100, classifierType, classifierParams, 0, perTrain, confusionMatrixInPercent, verbosity=verbosity)

    print "Selected params: {0:.4f}".format(bestParam)
...
def evaluateClassifier(features, ClassNames, nExp, ClassifierName, Params, parameterMode, perTrain=0.90, confusionMatrixInPercent=True, verbosity=True):
    '''
    ARGUMENTS:
        features:     a list ([numOfClasses x 1]) whose elements containt numpy matrices of features.
                each matrix features[i] of class i is [numOfSamples x numOfDimensions]
    ...
    for Ci, C in enumerate(Params):                # for each param value
                ...
                # print "Confusion Matrix for param {0:.5f}:".format(C)
                # printConfusionMatrix(CM, ClassNames, confusionMatrixInPercent)
    ...
    for i in range(len(PrecisionClassesAll)):
        ...
        if i == bestF1Ind:
            bestF1 = 100.0 * F1All[i]
            print "\t best F1",
        if i == bestAcInd:
            bestAccuracy = 100.0 * acAll[i]
            print "\t best Acc",
        print

    if parameterMode == 0:    # keep parameters that maximize overall classification accuracy:
        print "Confusion Matrix: Total: {}".format( numpy.sum(CMsAll[bestAcInd]) )
        printConfusionMatrix(CMsAll[bestAcInd], ClassNames, confusionMatrixInPercent)
        return (Params[bestAcInd], bestAccuracy)
    elif parameterMode == 1:  # keep parameters that maximize overall F1 measure:
        print "Confusion Matrix: Total: {}".format( numpy.sum(CMsAll[bestF1Ind]) )
        printConfusionMatrix(CMsAll[bestF1Ind], ClassNames, confusionMatrixInPercent)
        return (Params[bestF1Ind], bestF1)
...
def printConfusionMatrix(CM, ClassNames, confusionMatrixInPercent):
    '''
    ...
    for i, c in enumerate(ClassNames):
        ...
        for j in range(len(ClassNames)):
            if len(total_classes)-1 < i:
                total_classes.append(0.0)
            if confusionMatrixInPercent:
                total_classes[i] += 100.0 * CM[i][j] / numpy.sum(CM)
                print "\t{0:.2f}".format(100.0 * CM[i][j] / numpy.sum(CM)),
            else:
                total_classes[i] += CM[i][j]
                print "\t{0:.0f}".format(CM[i][j]),
        print
    print "{0:s}".format("Tot"),
    for k in range(len(total_classes)):
        print "\t{0:.2f}".format(total_classes[k]),
    print

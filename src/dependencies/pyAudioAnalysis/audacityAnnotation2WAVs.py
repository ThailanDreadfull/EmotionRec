...
def annotation2files(wavFile, csvFile, outputFolder):
    '''
    ...
    with open(csvFile, 'rb') as csvfile:
        ...
        for j, row in enumerate(reader):
            ...
            otFolder = outputFolder + '/' + row[2] + '/'
            if not os.path.isdir(otFolder):
                os.makedirs(otFolder)

            outputFile = outputFolder +'/'+row[2]+'/'+ str(wavFile).split('/')[-1]

            label = "%s_%s_%.2f_%.2f.wav" % (outputFile, row[2], T1, T2)
        ...
        for anFile in annotationFilesList:
            ...
            if not os.path.isfile(wavFile):
                ...
                if not os.path.isfile(wavFile):
                    ...
                    return
            annotation2files(wavFile, anFile, outputFolder)
